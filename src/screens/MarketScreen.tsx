      ...
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: insets.top + 10 }]} showsVerticalScrollIndicator={false}>
            ...
            >
              <View style={styles.itemInfo}>
                <View style={styles.itemNameRow}>
                  ...
                  <Text style={styles.itemName}>{tOr(`theme.${theme.id}`, theme.name)}</Text>
                </View>
                <Text style={styles.itemPrice}>{theme.price} {t('market.points')}</Text>
              </View>
              {owned ? (
                <TouchableOpacity style={styles.themeActionButton} onPress={() => handleSelectCardTheme(theme.id)}>
                  <LinearGradient colors={isActive ? ['#00c864', '#00a050'] : ['rgba(255,255,255,0.12)', 'rgba(255,255,255,0.06)']} style={styles.actionGradient}>
                ...
                <TouchableOpacity style={styles.themeActionButton} onPress={() => handlePurchaseTheme(theme.id, theme.price)}>
                  <LinearGradient colors={['#ffc107', '#e6a800']} style={styles.actionGradient}>
        ...
        {BG_THEMES.map((theme) => {
          ...
          return (
            ...
            >
              <View style={styles.itemInfo}>
                <View style={styles.itemNameRow}>
                  <LinearGradient colors={theme.gradient} style={styles.gradientDot} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} />
                  <Text style={styles.itemName}>{tOr(`theme.${theme.id}`, theme.name)}</Text>
                </View>
                <Text style={styles.itemPrice}>{theme.price} {t('market.points')}</Text>
              </View>
              {owned ? (
                <TouchableOpacity style={styles.themeActionButton} onPress={() => handleSelectBgTheme(theme.id)}>
                  <LinearGradient colors={isActive ? ['#00c864', '#00a050'] : ['rgba(255,255,255,0.12)', 'rgba(255,255,255,0.06)']} style={styles.actionGradient}>
                    <Text style={[styles.actionButtonText, isActive && styles.activeButtonText]}>
                      {isActive ? t('market.selected') : t('market.select')}
                    </Text>
        ...
        {CONSUMABLES.map((item) => {
          ...
          return (
            ...
            >
              <View style={styles.itemInfo}>
                <Text style={styles.itemName}>{tOr(`consumable.${item.id}`, item.name)}</Text>
                <Text style={styles.itemDesc}>{tOr(`consumable.${item.id}.desc`, item.desc)}</Text>
                <View style={styles.priceRow}>
                  <Text style={styles.itemPrice}>{item.price} {t('market.points')}</Text>
                  {qty > 0 && <Text style={styles.qtyBadge}>x{qty}</Text>}
                </View>
              </View>
              <TouchableOpacity style={styles.themeActionButton} onPress={() => handlePurchaseConsumable(item.id, item.price)}>
                <LinearGradient colors={['#ffc107', '#e6a800']} style={styles.actionGradient}>
                  <Text style={styles.buyButtonText}>{t('market.buy')}</Text>
                </LinearGradient>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 30, fontWeight: '900', color: '#ffffff', letterSpacing: 6, marginBottom: 20 },
  balanceCard: { borderRadius: 18, paddingVertical: 20, paddingHorizontal: 28, width: '100%', maxWidth: 340, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,199,7,0.3)', marginBottom: 30 },
  balanceInner: { flexDirection: 'column' },
  balanceLabel: { fontSize: 18, color: '#ffc107', letterSpacing: 3, fontWeight: '900' },
  balanceSubLabel: { fontSize: 10, color: 'rgba(255,199,7,0.5)', marginTop: 3, letterSpacing: 1 },
  balanceValue: { fontSize: 32, fontWeight: '900', color: '#ffc107' },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', marginLeft: 10, marginBottom: 12, marginTop: 8 },
  sectionDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#ffc107', marginRight: 8 },
  sectionTitle: { fontSize: 13, color: '#ffffff', letterSpacing: 2, fontWeight: 'bold' },
  itemCard: { borderRadius: 14, padding: 16, width: '100%', maxWidth: 340, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', marginBottom: 10 },
  itemInfo: { flex: 1, marginRight: 12 },
  itemNameRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
  colorDot: { width: 16, height: 16, borderRadius: 8, marginRight: 4, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  colorDotSmall: { width: 10, height: 10, borderRadius: 5, marginRight: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  gradientDot: { width: 16, height: 16, borderRadius: 8, marginRight: 8, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  itemName: { fontSize: 15, fontWeight: 'bold', color: '#ffffff' },
  itemDesc: { fontSize: 11, color: '#a0a0b0', marginTop: 2 },
  priceRow: { flexDirection: 'row', alignItems: 'center', marginTop: 4 },
  itemPrice: { fontSize: 12, color: '#ffc107', marginTop: 2 },
  qtyBadge: { fontSize: 12, color: '#00c864', fontWeight: 'bold', marginLeft: 10, backgroundColor: 'rgba(0,200,100,0.15)', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 },
  themeActionButton: { borderRadius: 10, overflow: 'hidden' },
  actionGradient: { paddingHorizontal: 16, paddingVertical: 10, minWidth: 85, alignItems: 'center' },
  buyButtonText: { color: '#1a1a2e', fontSize: 11, fontWeight: 'bold', letterSpacing: 1 },
  actionButtonText: { color: '#a0a0b0', fontSize: 11, fontWeight: 'bold', letterSpacing: 1 },
  ...
  modalText: { fontSize: 14, color: '#a0a0b0', textAlign: 'center', lineHeight: 22, marginBottom: 16 },
  modalHighlight: { color: '#ffc107', fontWeight: 'bold' },
  modalInfo: { backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 12, padding: 14, width: '100%', marginBottom: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
  modalInfoRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 },
  modalInfoLabel: { fontSize: 13, color: '#a0a0b0' },
  modalInfoValue: { fontSize: 15, fontWeight: 'bold', color: '#ffc107' },
  modalInfoValueRed: { fontSize: 15, fontWeight: 'bold', color: '#e94560' },
  modalButton: { borderRadius: 12, overflow: 'hidden', width: 160 },
